...
2. **添加穿透配置**  
   ...
   - **启用**：控制此条穿透配置是否生效。关闭后，访问绑定域名时 Dashboard 会拒绝该穿透请求，适合临时停用某个内网服务而不删除配置。
...
2. **并发限制**
   ...
   每条活跃的内网穿透连接都会占用目标服务器的一条 IOStream。同一服务器最多同时承载 40 条 IOStream，此上限还包括在线终端、文件管理和 MCP 文件传输；达到上限时，新连接会被拒绝，关闭不用的会话后即可重试。

---
description: 哪吒监控 V2 Dashboard 的 Nginx 与 Caddy 反向代理配置，正确转发普通 HTTP、WebSocket 和 Agent gRPC，并处理 TLS 与认证头。
outline: deep
---

# Dashboard 反向代理配置

哪吒监控 V2 不区分 Dashboard Web 和 Agent gRPC 端口，访问与通信均可通过默认的 `8008` 端口。

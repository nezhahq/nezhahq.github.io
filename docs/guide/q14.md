...
- **`dashboard_host`**
  ...
  - Dashboard 对外访问并接收 OAuth2 回调的 `域名/IP[:端口]`，不要包含协议或路径；使用 HTTP/HTTPS 默认端口时可省略端口。
  - 使用 OAuth2 时建议显式设置；当 Dashboard 位于反向代理后方，或公网访问地址与 Agent 对接地址不同时尤其如此。详情见 [`dashboard_host`](/configuration/dashboard.html#dashboard-host)。
...
哪吒的 Callback URI 路径为 `/api/v1/oauth2/callback`。在 OAuth2 提供方填写完整地址，例如 `https://nezha.example.com/api/v1/oauth2/callback`，并确保域名与 `dashboard_host`（或可信的 `reserved_hosts`）一致。
...
已经绑定的 OAuth 2.0 账户可在个人信息页解除绑定。解除绑定接口属于个人账户自我管理操作，只允许当前登录会话调用，PAT 会被拒绝。

如果当前用户启用了“禁止密码登录”，且只剩下一个 OAuth 2.0 绑定关系，则无法删除最后一个绑定关系，以免账户无法登录。

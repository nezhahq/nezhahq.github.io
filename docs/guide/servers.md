...
- **并发限制**：同一用户最多同时占用 20 条在线终端与文件管理 IOStream；同一目标服务器最多同时承载 40 条 IOStream（包括在线终端、文件管理、内网穿透和 MCP 文件传输）。达到上限时请关闭不用的会话后重试。
...
在线终端与文件管理共用用户并发配额；同一用户最多同时占用 20 条相关 IOStream。目标服务器还受 40 条总 IOStream 上限约束，该上限同时计入内网穿透和 MCP 文件传输。

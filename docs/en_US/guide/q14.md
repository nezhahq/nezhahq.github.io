...
- **`dashboard_host`**
  ...
  - Set it explicitly when using OAuth2, especially when Dashboard is behind a reverse proxy or its public address differs from the Agent connection address. See [`dashboard_host`](/en_US/configuration/dashboard.html#dashboard-host) for details.

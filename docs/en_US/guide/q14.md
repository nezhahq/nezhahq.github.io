...
```yaml
dashboard_host: "nezha.example.com"
oauth2:
...
- **`dashboard_host`**
  ...
  - The public `domain/IP[:port]` used to access Dashboard and receive OAuth2 callbacks. Do not include a scheme or path; omit the port when using the default HTTP/HTTPS port.
  - Set it explicitly when using OAuth2, especially when Dashboard is behind a reverse proxy or its public address differs from the Agent connection address. See [`dashboard_host`](/en_US/configuration/dashboard.html#dashboard_host) for details.
...
The Nezha callback path is `/api/v1/oauth2/callback`. Enter the full URL at the OAuth2 provider, for example `https://nezha.example.com/api/v1/oauth2/callback`, and make sure its hostname matches `dashboard_host` (or a trusted `reserved_hosts` entry).
...
5. Add your Dashboard Callback address to `Redirect URLs`:
...
Bound OAuth 2.0 accounts can be unbound from the profile page. The unbind API is a personal-account self-management action, so it only accepts the current login session and rejects PATs.

If the current user has enabled "Reject Password Login" and only one OAuth 2.0 binding remains, the last binding cannot be removed, preventing the account from becoming unable to log in.
